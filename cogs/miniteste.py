...
class Miniteste(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.api_miniteste_resposta = "{API}/api/v1/miniteste/adicionar/resposta"
    ...
    async def miniteste(self, interaction: discord.Interaction, teste: str):
        try:
            ...
            if response.status_code == 200:
                ...
                await interaction.response.send_message(f"{interaction.user}, este teste não existe ou ocorreu um erro ao recuperar os dados. 🔺", ephemeral=True)
        except ValueError:
            await interaction.response.send_message(f"{interaction.user}, a variavel do teste deve ser um valor numérico! 🔺", ephemeral=True)
