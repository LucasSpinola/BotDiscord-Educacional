...
class DarPresenca(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        self.api_presenca = "{API}/api/v1/presenca/marcar-presenca/"
        self.api_permissao = "{API}/api/v1/permissao/pegar_permissao"
        self.token = os.getenv('API_TOKEN')
        
    async def marcar_presenca(self, sigla: str, matricula: int, aula: str, data: str):
        headers = {'Authorization': f'Bearer {self.token}', 'Content-Type': 'application/json'}
        data = {"sigla": sigla, "matricula": matricula,"aula": aula,"data": data}
    ...
    async def registrar_presenca(self, interaction: discord.Interaction, matricula: int, sigla: str, aula: str ,data: str):
        id_user = str(interaction.user.id)
        check_permission = await self.check_permission(id_user)
        
        if check_permission:
            presenca = await self.marcar_presenca(sigla, matricula, aula, data)
